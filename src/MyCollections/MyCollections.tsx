          ...
          enqueueSnackbar(`Preparing to list`,{ variant: 'info' });
            ...
            requestListing(collectionBoardingInfo)
              .then(([tx, listingAddress]) => {
                ...
                enqueueSnackbar(`Listing address ${tx} created with transaction ${listingAddress}`,{ variant: 'info' });})
              .catch(e => {
        ...
        <>
          <Box 
            ...
            <TabPanel value={value} index={0}>
              ...
              {approvedListings && <ApprovedTable deny={denyListing} enable={setEnableListing} isAdmin={isAdminConnected === undefined ? false : isAdminConnected} info={approvedListings}/>}
            </TabPanel>
            <TabPanel value={value} index={1}>
              {!pendingListings && <p>Loading...</p>}
              {pendingListings && <PendingTable wallet={anchorWallet.publicKey} refund={requestListingRefund} approve={approveListing} isAdmin={isAdminConnected === undefined ? false : isAdminConnected} info={pendingListings}/>}
            </TabPanel>
          </Box>
        </>
      :
