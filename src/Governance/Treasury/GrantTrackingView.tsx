...
function RecipientActivity({wallet,mint,since,grants,onClose}:{wallet:string;mint:string;since:number;grants:Grant[];onClose:()=>void}) {
  ...
  const visibleEvents=events.filter(e=>filter==='all'||e.type===filter).sort((a,b)=>b.timestamp-a.timestamp);
  return <Box sx={{mt:3,p:2,border:'1px solid rgba(255,255,255,0.15)',borderRadius:2}}>
    ...
    {loaded && <>
      ...
      <Stack direction="row" spacing={1} sx={{mb:2}}>{[['all','All activity'],['swap','Swaps'],['transfer','Transfers']].map(([value,label])=><Chip key={value} label={label} clickable color={filter===value?'primary':'default'} variant={filter===value?'filled':'outlined'} aria-pressed={filter===value} onClick={()=>setFilter(value)}/>)}</Stack>
      <TableContainer><Table size="small"><TableHead><TableRow><TableCell>Date</TableCell><TableCell>Activity</TableCell><TableCell align="right">Community tokens</TableCell><TableCell>Transaction</TableCell></TableRow></TableHead><TableBody>
        {visibleEvents.map(e=><TableRow key={`${e.signature}:${e.type}`}><TableCell>{new Date(e.timestamp*1000).toLocaleString()}</TableCell><TableCell>{e.type==='swap'?'Swap':'Transfer'}</TableCell><TableCell align="right">{new BigNumber(e.amount).toFormat()}</TableCell><TableCell>{txLink(e.signature)}</TableCell></TableRow>)}
        {!visibleEvents.length && <TableRow><TableCell colSpan={4}>No matching activity in the loaded history.</TableCell></TableRow>}
      </TableBody></Table></TableContainer>
    </>}
...
export default function GrantTrackingView({mint,wallets,grantors=[]}:{mint:string;wallets:string[];grantors?:string[]}) {
  ...
  return <Accordion sx={{my:2,background:'rgba(255,255,255,0.03)'}}>
    ...
    <AccordionDetails>
      ...
      {selected && <RecipientActivity key={`${active}:${selected}:${since}:${mint}`} wallet={selected} mint={mint} since={since} grants={recipientGrants} onClose={()=>setSelected('')}/>}
    </AccordionDetails>
  </Accordion>;
}
