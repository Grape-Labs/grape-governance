...
import { InstructionDataWithHoldUpTime } from '../../../Proposals/createProposalInstructionsV0';
...
export default function JupiterSwapView(props: any) {
    ...
    async function setupSwap() {
        ...
        const quoteResponse = await (
            await fetch('https://quote-api.jup.ag/v6/quote?inputMint='+tokenMint+'&outputMint='+toMintAddress+'&amount='+inAmount+'&slippageBps=50&maxAccounts=30'
            //await fetch('https://quote-api.jup.ag/v6/quote?inputMint='+tokenMint+'&outputMint='+toMintAddress+'&amount='+inAmount+'&slippageBps=50'
            )
          ).json();
        ...
        if (quoteResponse){
            ...
            if ((swapInstructions)&&(transactions)){
                ...
                const deserializeInstruction = (instruction) => {
                 ...
                 const swapTransactionInstruction = [
                ...
                console.log("swapInstructions (actual instructions for transaction):", swapTransactionInstruction);
                /*serializedTransferToReceiptIxs.push(//swapTransactionInstruction,
                    swapInstructionPayload
                    //serializeInstructionToBase64(swapInstructionPayload)
                )*/
                //serializedTransferToReceiptIxs.push(serializeInstructionToBase64(swapTransactionInstruction));
                //serializedTransferToReceiptIxs.push(...swapTransactionInstruction.map(instruction => serializeInstructionToBase64(instruction)));
                
                // Use map to convert each instruction to a serialized string and push into the array
                swapTransactionInstruction.forEach(instruction => {
            ...
            //tx.add(new TransactionInstruction(instructions));
                ...
                */
                console.log('setInstructionsDataWithHoldUpTime sent:'+JSON.stringify(instructions));
